# Практическая модель интеграции AI в IT-проекты аутсорс-компании

Версия: 1.0  
Дата: 21 августа 2026 г.  
Назначение: руководство для AI-интегратора, engineering-менеджмента, архитекторов, разработчиков, QA, DevOps/SRE, аналитиков и технических писателей.

## 1. Резюме для руководителя

Рекомендуемая модель — не «добавить AI в клиентский репозиторий», а создать **внешний AI-контур разработки**, который помогает команде производить обычные, проверяемые и поддерживаемые инженерные результаты.

Клиент получает:

- код, тесты, документацию, спецификации и ADR, которые имеют самостоятельную ценность без AI;
- стандартную историю изменений и обычный процесс code review;
- качество, безопасность, трассируемость требований и понятную ответственность людей.

Клиентский репозиторий не должен содержать служебные AI-артефакты:

- промпты, транскрипты и память агента;
- конфигурации конкретных моделей и IDE-ассистентов;
- временные планы, черновики, результаты внутренних AI-eval;
- подписи «generated by AI», AI co-author trailers и автоматические рекламные формулировки;
- служебные каталоги `.claude`, `.cursor`, `.specify`, файлы `AGENTS.md`, `CLAUDE.md` и аналоги, если они не согласованы с заказчиком как часть поставки.

При этом «AI-neutral repository» не означает тайное или запрещённое использование AI. Применение AI должно быть разрешено договором, политикой безопасности, требованиями к интеллектуальной собственности и правилами обработки данных. Если заказчик требует раскрытия, запретил передачу кода внешней модели или регулируемая информация не может покидать определённый контур, это требование имеет приоритет.

Базовый производственный поток:

> Intake → классификация данных и риска → спецификация → уточнение человеком → технический план → малые вертикальные задачи → реализация → автоматические проверки → независимый review → PR → измерение результата → очистка временных AI-артефактов

Основная рекомендация по Spec-Driven Development: использовать **spec-anchored**, а не «spec-as-source». Спецификация остаётся проверяемым источником намерения и acceptance criteria, но код не считается механически генерируемым продуктом, а разработчик не снимает с себя ответственность.
...
3. **Клиентский репозиторий AI-neutral.** В него попадает только то, что полезно для эксплуатации и сопровождения продукта независимо от инструмента создания.
...
## 3. Целевая архитектура: четыре контура

### 3.1. Контур клиентской поставки

Содержит клиентский репозиторий, issue tracker, CI/CD и документы, являющиеся частью продукта. Допустимы только устойчивые инженерные артефакты:
...
### 3.2. Приватный контур AI-enablement аутсорсера
...
Он **не должен становиться теневой копией клиентского репозитория**. Клиентский код, документы и логи нельзя постоянно дублировать туда без договорного основания. Проектный контекст либо остаётся в одобренном инструменте с заданной retention policy, либо существует локально и временно.

### 3.3. Управляемый execution-контур
...
- запрет personal/free AI accounts для клиентского кода.

Конфигурация агента загружается **из-за пределов клиентского репозитория**: через управляемый профиль IDE, launcher, dev-container image, MDM, внутренний CLI или внешний workspace. Символические ссылки и один лишь `.gitignore` — слабая защита: агент или человек может всё равно прочитать или случайно добавить файл.

### 3.4. Governance-контур
...
## 4. Классификация артефактов

| Класс | Примеры | Где хранить | В клиентский Git |
|---|---|---|---|
| A. Поставляемые | код, тесты, API spec, ADR, runbook | клиентские системы | да |
...
## 5. Три режима допуска AI к проекту

### Green lane

Для публичного или синтетического контекста, boilerplate, общих вопросов, обучения, генерации тестовых данных без клиентской информации. Разрешены корпоративно одобренные SaaS-модели.

### Amber lane

Для конфиденциального клиентского кода. Нужны:
...
Значимые долгоживущие решения оформляются обычным ADR в клиентском формате. Не нужно писать «AI предложил»; документирует и принимает решение инженер.
...
Запрещены недостоверные утверждения вроде «fully tested», если проверки не выполнены. Перед push и перед клиентским handoff выполняется **AI Artifact Leak Check**.

### Шаг 10. Обратная связь и очистка

- фиксируются принятый результат, rework, дефекты и фактическое время;
- полезное обобщённое знание переносится во внутренний playbook без клиентских секретов;
...
Не просить модель «объяснить весь репозиторий». Сначала построить карту модулей, entry points, dependency graph, critical flows и тестовые команды; затем проверять каждый вывод по коду. Результатом может быть полезный клиенту architecture overview, но транскрипт исследования остаётся внутренним.
...
## 9. Definition of Ready и Definition of Done
...
Сканер не должен слепо запрещать слова `AI`, `GPT` или `model`: они могут быть легитимной частью клиентского продукта. Контроль ищет конкретные служебные артефакты и поддерживает project-specific allowlist. `.gitignore` — только удобство, не контроль безопасности.

## 11. Security и supply-chain controls
...
## 12. Выбор AI-инструмента: обязательная анкета
...
- утечки AI-артефактов в client branch/release;
...
- согласовать, что client contract всегда выше внутреннего playbook.
...
## 17. Пакет материалов AI-интегратора

Минимальный reusable kit:

1. AI Usage Policy.
2. Client Contract & Disclosure Checklist.
...
12. Golden Task/Eval Suite без клиентских секретов.
...
- Не устанавливать полный AI-framework в клиентский репозиторий «по умолчанию».
- Не копировать клиентский репозиторий во внутреннюю knowledge base без договора.
...
## 19. Контрольные вопросы для адаптации под компанию
...
2. внешний по отношению к клиентскому Git AI-контур;
...
Это сохраняет главное преимущество AI — быстрое исследование, декомпозицию, генерацию вариантов и автоматизацию рутинной работы — не превращая клиентскую кодовую базу в зависимый от конкретной модели или поставщика набор служебных файлов.
